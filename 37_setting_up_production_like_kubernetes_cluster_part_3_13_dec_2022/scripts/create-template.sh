...
if [[ -z "$Pz_VM_TEMPLATE_ID" ]]; then
  printf "${WARNCOLOR}Variable ${MAIN_MSGCOLOR}Pz_VM_TEMPLATE_ID${WARNCOLOR} does not exist\n"
  printf "Check your copy of ${MAIN_MSGCOLOR}.env.example${WARNCOLOR} - file and run command \"${MAIN_MSGCOLOR}set -a; source ./.env.example; set +a${WARNCOLOR}\"\n${NOCOLOR}"
  exit 1
fi

if grep -q "\"$Pz_VM_TEMPLATE_ID\"" /etc/pve/.vmlist ; then
  printf "${WARNCOLOR}Template with ID ${MAIN_MSGCOLOR}$Pz_VM_TEMPLATE_ID${WARNCOLOR} exists\n"
  printf "Do you want to remove template with ID ${MAIN_MSGCOLOR}$Pz_VM_TEMPLATE_ID${WARNCOLOR} ?\n"
  echo -n "y/n :${NOCOLOR}"
  read -r res
  if [ "$res" == 'y' ]; then
    ./destroy-template.sh
...
#
if [ "$Pz_DISK_FORMAT" == 'raw' ] || [ "$Pz_DISK_FORMAT" == 'qcow2' ]; then
